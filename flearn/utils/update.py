...
from flearn.utils.qsgd_util import quantize_state_dict
import copy
...
class LocalUpdate(object):
    ...
    def update_weights(self, model, global_round, layers_diff_epochs=[], global_mask=None, error=None, alpha_up=None):
        ...
        if self.args.hypernet_comm == True and alpha_up is not None:

            print("\nhypernet_upload", end=" ")
            topK_hypernet(record_w, cur_w, alpha_up, mask)
            recover_model_from_mask(cur_w, record_w, mask)
        ...
        if self.args.is_qsgd == True:
            print("\nquan_upload", end=" ")
            u = quantize_state_dict(u, self.args.quan_bits)
...
if __name__ == "__main__":
    flops, params = get_flops(model="alexnet", config=None, dataset="cifar100")
    print(int(flops))
    print(int(params))
