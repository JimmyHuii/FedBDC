...
from linear import DenseLinear

from conv2d import DenseConv2d
from ..utils.model_utils import is_fc, is_conv
...
class ResNet(BaseModel):
    def __init__(self, in_channel=3, dict_module: dict = None, block=BasicBlock, layers=(2, 2, 2, 2), num_classes=1000,
...
def resnet18(in_channel=3, num_classes=10) -> ResNet:
...
class ResidualBlock(nn.Module):
    def __init__(self, inchannel, outchannel1, outchannel2, stride=1):
        ...
        self.left = nn.Sequential(
            nn.Conv2d(inchannel, outchannel1, kernel_size=3, stride=stride, padding=1, bias=True),
            nn.BatchNorm2d(outchannel1),
            nn.ReLU(inplace=True),
            nn.Conv2d(outchannel1, outchannel2, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(outchannel2)
        )
        self.shortcut = nn.Sequential()
        if stride != 1 or inchannel != outchannel2:
            self.shortcut = nn.Sequential(
                nn.Conv2d(inchannel, outchannel2, kernel_size=1, stride=stride, bias=True),
                nn.BatchNorm2d(outchannel2)
            )
...
class ResNet2(nn.Module):
    def __init__(self, ResidualBlock, in_channel=3, num_classes=10, config=None):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channel, config[0], kernel_size=3, stride=1, padding=1, bias=True),
...
if __name__ == "__main__":
    ...
    inputs = torch.ones((64, 1, 32, 32))
    outputs = model(inputs)
    print(f"mnist: {outputs.shape}")

    rank = [0, torch.linspace(1, 64, steps=64)]
